...
1. 提交代码并推送到远端
2. 如需正式发新版，再更新 `package.json` 中的版本号
3. 创建并推送与当前版本对应的 Git tag，例如 `v1.2.0`
...
- 默认使用本地口令登录
- 支持自定义密码与恢复码重置
- 默认密码和恢复码仍保留在源码常量中，便于本地维护和手动修改
- 自定义密码仅用于本地访问控制，不等同于服务端鉴权或对抗性安全方案
- 自定义密码本地存储时已避免明文落盘
- Electron 模式下启用了 `contextIsolation: true`
- `nodeIntegration` 关闭
- 通过 preload 暴露白名单 API
- 主进程会在首屏加载前注册外部请求拦截
- 开发与生产环境使用不同强度的 CSP，生产环境默认不放开 `unsafe-eval`
